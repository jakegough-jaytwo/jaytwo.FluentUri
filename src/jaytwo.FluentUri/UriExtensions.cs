...
public static class UriExtensions
{
    ...
    {
        var escapedArgs = formatArgs?.Select(Uri.EscapeDataString).ToArray() ?? Array.Empty<string>();
    ...
    {
        var escapedArgs = formatArgs?.Select(x => Uri.EscapeDataString($"{x}")).ToArray() ?? Array.Empty<string>();
    ...
    {
        return WithQueryParameter(uri, key, values?.Cast<object?>().ToArray() ?? Array.Empty<string>());
    }
